  ...
  return <div className="heritage-home">
    <section className={`heritage-hero ${intro ? 'intro-playing' : ''}`}>
      ...
      <div className="hero-map-column">
        ...
        <div className="map-bottom"><span>Drag to orbit · Scroll to zoom · Select a landmark</span><button onClick={() => setClassic(v => !v)} aria-expanded={classic}><Layers size={14}/>{classic ? 'Hide state explorer' : 'State explorer'} <ArrowUpRight size={14}/></button></div>
    ...
    <section className="featured-section" ref={destinations} id="destinations">
      ...
      <div className="featured-grid">{FEATURED_IDS.map((id, index) => { const item = HERITAGE_LANDMARKS.find(l => l.id === id)!; return <button key={id} className={`monument-card card-${id}`} onClick={() => launch(id)}>
        <div className="card-art" aria-hidden="true">{id !== 'amber-fort' && <img src={item.thumbnailUrl} alt="" loading="lazy" onError={e => { e.currentTarget.style.display = 'none'; }}/>}<span className="card-silhouette">{id === 'amber-fort' ? 'आमेर' : item.hindiName.split('(')[0]}</span><span className="card-index">0{index + 1}</span><span className="card-tour-badge">{id === 'golden-temple' ? 'EXTERNAL 360°' : id === 'taj-mahal' ? '360° TOUR' : 'JAIPUR 360° TOUR'}</span></div>
